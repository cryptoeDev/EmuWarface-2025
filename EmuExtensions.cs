...
namespace EmuWarface
{
    public static class EmuExtensions
    {
        ...
        internal static Int64 ParseSeconds(string data)
        {
            Int64 seconds = 0;
            if (data.Contains("month") || data.Contains("m"))
            {
                seconds = Int64.Parse(data.Replace("month", "").Replace("m", "")) * 60 * 60 * 24 * 30;
            }
            else if (data.Contains("day") || data.Contains("d"))
            {
                seconds = Int64.Parse(data.Replace("day", "").Replace("d", "")) * 60 * 60 * 24;
            }
            else if (data.Contains("hour") || data.Contains("h"))
            {
                seconds = Int64.Parse(data.Replace("hour", "").Replace("h", "")) * 60 * 60;
            }
            return seconds;
        }
        ...
        public static void UpdateOnline()
        {
            lock (Server.Clients)
            {
                File.WriteAllText(EmuConfig.Settings.OnlinePath, Server.Clients.Where(x => !x.IsDedicated && x.Presence.HasFlag(PlayerStatus.Online)).Count().ToString());
            }
        }
        ...
        public static void UpdateRoom()
        {
            foreach (var channel in Server.Channels)
            {
                lock (channel.Rooms)
                {
                    File.WriteAllText(EmuConfig.Settings.RoomPath, channel.Rooms.Count.ToString());
                }
            }
        }
        ...
        public static bool MatFilter(string input)
        {
            ...
            foreach (string word in EmuConfig.ObsceneWords)
            {
                if (input.Contains(word))
                {
                    if (input.Contains("продам"))
                    {
                        API.SendAdmins("WARNING MESSAGE: " + input);
                    }
                    return false;
                }
            }
